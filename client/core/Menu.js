...
const isActive = (history, path) => {
  if (history.location.pathname == path) return { color: "#F2B705" };
  else return { color: "#ffffff" };
};
const isPartActive = (history, path) => {
  if (history.location.pathname.includes(path)) return { color: "#F2B705" };
  else return { color: "#ffffff" };
};
    ...
    <Toolbar>
      ...
      <div>
        ...
        <Link to="/auctions/all">
          <Button style={isActive(history, "/auctions/all")}>Auctions</Button>
        </Link>
        <Link to="/cart">
